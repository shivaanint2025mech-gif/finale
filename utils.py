...
def send_event_emails(event, participants, flier_path=None):
    """Send email with flier (as attachment or link) and feedback link"""
...
def import_students_from_excel(filepath):
    """Import students from Excel with columns: name, email, roll_number, joining_year"""
...
def import_faculty_from_excel(filepath):
    """Import faculty from Excel with columns: name, email, department"""
    df = pd.read_excel(filepath)
